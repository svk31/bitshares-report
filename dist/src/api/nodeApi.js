  ...
  return new Promise(function(resolve, reject) {
    ...
    FetchChain(
      ...
      .then(function(result) {
        ...
        account.call_orders.forEach(function(c) {
          var balanceIndex = account.balances.findIndex(function(b) {
            return b.asset_type === c.call_price.base.asset_id;
          });
          if (balanceIndex !== -1) {
            var newBalance =
              parseInt(account.balances[balanceIndex].balance, 10) +
...
function getAssetData(asset) {
  return new Promise(function(resolve, reject) {
    ...
    FetchChain(
      ...
      .then(function(result) {
        console.log("FetchChain", asset, result);
        var a = result.toJS();
        assetData[asset] = {
          name: a.symbol.replace(/OPEN\.|BRIDGE\.|RUDEX\.|GDEX\.|BLOCK\./, ""),
          precision: a.precision
        };
        resolve(assetData[asset]);
      })
      .catch(function(err) {
        console.log("getAssetData failed", err);
        reject();
      });
  });
}
