...
var parser = parse({delimiter: ','}, function (err, data) {
        ...
        */
        ...
        if (previous[key] && orderNumber === previous[key].data[8]) {
            ...
            entriesToSum.forEach(function(idx) {
                asObject[indice][idx] = parseFloat(asObject[indice][idx]) + parseFloat(previous[key].data[idx]);
            })
    ...
    console.log("Removed", originalLength - Object.keys(asObject).length, "trade entries by order grouping");
...
var parser = parse({delimiter: ','}, function (err, data) {
    ...
    fs.open(`output/poloniexDepositHistory.csv`, 'w', (err, fd) => {
        ...
        data.forEach(d => {
            if (d[2] !== "Amount")
                withdrawOut.push(["Deposit", d[2], d[1], "", "", "", "", "Poloniex", "", d[3], d[0]]);
        })
...
var parser = parse({delimiter: ','}, function (err, data) {
    ...
    fs.open(`output/poloniexWithdrawHistory.csv`, 'w', (err, fd) => {
        ...
        data.forEach(d => {
            if (d[2] !== "Amount")
                depositOut.push(["Withdrawal", "", "", d[2], d[1],  "", "", "Poloniex", "", d[3], d[0]]);
        })
